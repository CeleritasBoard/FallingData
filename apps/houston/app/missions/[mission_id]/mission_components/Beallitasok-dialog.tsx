...
interface BeallitasokDialogProps {
    ...
    initialData?: {
        ...
        meresHossza: number
        okezas: boolean
        headerPacket: boolean
        folytatasHaMegtelik: boolean
    }
}
...
export function BeallitasokDialog({
    ...
    // Form state
    const [tipus, setTipus] = useState(initialData?.tipus ?? "MAX_HITS")
    const [meresiTartomany, setMeresiTartomany] = useState<[number, number]>([
        initialData?.meresiMin ?? 189,
        initialData?.meresiMax ?? 3300,
    ])
    const [sampling, setSampling] = useState(initialData?.sampling ?? 5)
    const [felbontas, setFelbontas] = useState(initialData?.felbontas ?? 128)
    const [meresHossza, setMeresHossza] = useState(initialData?.meresHossza ?? 4000)
    const [okezas, setOkezas] = useState(initialData?.okezas ?? true)
    const [headerPacket, setHeaderPacket] = useState(initialData?.headerPacket ?? true)
    const [folytatasHaMegtelik, setFolytatasHaMegtelik] = useState(
        initialData?.folytatasHaMegtelik ?? false
    )
    ...
    async function handleSave() {
        ...
        const response = await apiFetch(`/missions/${mission_id}/settings`, "POST", formData);
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Módosítsd a küldetésed részleteit, majd ha készen vagy mentsd el őket.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="flex flex-col gap-6 py-4">
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <RadioGroup value={tipus} onValueChange={setTipus}>
                            <div className="flex items-center gap-2">
                                ...
                                <Label htmlFor="max_hits" className="font-normal cursor-pointer">
                                    MAX_HITS
                                </Label>
                            </div>
                            <div className="flex items-center gap-2">
                                <RadioGroupItem value="MAX_TIME" id="max_time" />
                                <Label htmlFor="max_time" className="font-normal cursor-pointer">
                                    MAX_TIME
                                </Label>
                            </div>
                        </RadioGroup>
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Slider
                                ...
                                onValueChange={(value) => setMeresiTartomany(value as [number, number])}
                                min={0}
                                max={3300}
                                step={1}
                                className="flex-1"
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        <div className="flex items-center gap-2">
                            <Checkbox
                                id="headerpacket"
                                checked={headerPacket}
                                onCheckedChange={(checked) => setHeaderPacket(checked === true)}
                            />
                            <Label htmlFor="headerpacket" className="font-normal cursor-pointer">
                                Header packet
                            </Label>
                        </div>
                        ...
                        <div className="flex items-start gap-2">
                            <Checkbox
                                id="folytatas"
                                checked={folytatasHaMegtelik}
                                onCheckedChange={(checked) => setFolytatasHaMegtelik(checked === true)}
                                className="mt-0.5"
                            />
                            <div className="flex flex-col">
                                <Label htmlFor="folytatas" className="font-normal cursor-pointer">
