...
export function PacketekCard({ data }: { data: PacketItem[] | null }) {
  ...
  return (
    <Card className="max-h-[440px] w-full">
      <CardHeader>
        <CardTitle className="text-lg">Packetek</CardTitle>
      </CardHeader>
      <CardContent className="overflow-auto h-full w-full">
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {data.map((item, index) => (
              <TableRow key={`${item.id}-${index}`} className="border-0">
                ...
                <TableCell className="px-2 py-1.5 text-sm font-mono text-foreground">
                  {item.packet}
                </TableCell>
                <TableCell className="px-2 py-1.5">
                  <ExternalLink
                    className="h-3.5 w-3.5 text-muted-foreground"
                    onClick={() => window.open(`/packets/${item.id}`, "_blank")}
                  />
                </TableCell>
              </TableRow>
            ))}
