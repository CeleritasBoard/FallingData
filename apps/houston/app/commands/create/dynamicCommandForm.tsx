...
import { Form, FormControl, FormField, FormItem, FormLabel } from "@workspace/ui/src/components/form.tsx"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@workspace/ui/src/components/select.tsx"
...
async function LoadDataView(allData: allSubmitData): Promise<string | null> {
    try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_HOST}/commands`, {
            method: "PUT",
            headers: {
                "Content-Type": "commands/json",
            },
            body: JSON.stringify(allData),
        })

        if (!response.ok) {
            throw new Error("Hiba történt a parancs létrehozásakor")
        }
...
const NO_PARAM_COMMANDS = ["FORCE_STATUS_REPORT", "REQUEST_SELFTEST", "RESET", "RESTART", "SAVE", "STOP"]

export function DynamicCommandForm({ submittedData, onAbort }: DynamicCommandFormProps) {
    ...
    const setScaleForm = useForm<SetScaleValues>({
        ...
        defaultValues: {
            measurement_range_min: 2000,
            measurement_range_max: 3000,
            resolution: 8,
            sample: 125,
        },
    })
    ...
    const onSetScaleSubmit = async (data: SetScaleValues) => {
        console.log("[v0] SET_SCALE Form submitted:", { ...submittedData, ...data })
    ...
    const onRequestMeasurementSubmit = async (data: RequestMeasurementValues) => {
        console.log("[v0] Request measurement Form submitted:", { ...submittedData, ...data })
    ...
    switch (submittedData.type) {
        case "SET_DUR":
            return (
                <div className="mt-8 p-6 border border-border rounded-lg">
                    ...
                    <Form {...setDurForm}>
                        <form onSubmit={setDurForm.handleSubmit(onSetDurSubmit)} className="space-y-6">
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input type="number" className="w-[250px]" placeholder="100" {...field} />
                                        </FormControl>
                                    </FormItem>
                                )}
                            />
        ...
        case "SET_SCALE":
            return (
                <div className="mt-8 p-6 border border-border rounded-lg">
                    ...
                    <Form {...setScaleForm}>
                        <form onSubmit={setScaleForm.handleSubmit(onSetScaleSubmit)} className="space-y-6">
                            <FormField
        ...
        case "REQUEST_MEASUREMENT":
            return (
                <div className="mt-8 p-6 border border-border rounded-lg">
                    <h2 className="text-2xl font-semibold mb-4">Request measurement paraméterek</h2>
                    <Form {...requestMeasurementForm}>
                        <form onSubmit={requestMeasurementForm.handleSubmit(onRequestMeasurementSubmit)} className="space-y-6">
                            <FormField
        ...
        case "STOP":
            return (
                <div className="mt-8 p-6 border border-border rounded-lg">
                    ...
                    <p className="text-muted-foreground">Kérjük várjon, a parancs feldolgozás alatt.</p>
                </div>
            )
        ...
        default:
            return (
                <div className="mt-8 p-6 border border-border rounded-lg">
                    <h2 className="text-2xl font-semibold mb-4">{submittedData.type} Paraméterek</h2>
                    <p className="text-muted-foreground">Ez a parancs típus még nincs implementálva.</p>
                    <Button type="button" variant="destructive" onClick={onAbort} className="mt-4">
                        Megszakítás
                    </Button>
                </div>
            )
