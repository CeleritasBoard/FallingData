...
export const requestMeasurementSchema = z.object({
  timestamp: z.transform(Number).pipe(z.number().min(0).max(4294967295)),
...
export const setDurSchema = z.object({
  repetitions: z.transform(Number).pipe(z.number().min(1).max(63)),
  mode: z.enum(["MAX_HITS", "MAX_TIME"]),
  okaying: z.boolean(),
  duration: z.transform(Number).pipe(z.number().min(1).max(65535)),
  breaktime: z.transform(Number).pipe(z.number().min(1).max(65535)),
});

export const setScaleSchema = z
  .object({
    lowerThreshold: z.transform(Number).pipe(z.number().min(0).max(4094)),
    upperThreshold: z.transform(Number).pipe(z.number().min(1).max(4095)),
    resolution: z.transform(Number).pipe(
      z
        .number()
        .refine(
          (val) =>
            RESOLUTION_VALUES.includes(
              val as (typeof RESOLUTION_VALUES)[number],
            ),
          {
            message:
              "Resolution must be one of: 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024",
          },
        ),
    ),
    sample: z.transform(Number).pipe(z.number().min(1).max(255)),
  })
  .refine((data) => data.lowerThreshold < data.upperThreshold, {
    message:
      "A mérési tartomány minimumának kisebbnek kell lennie, mint a maximum",
    path: ["upperThreshold"],
  });
